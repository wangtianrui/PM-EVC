...
def main(args):
    if os.path.exists(args.csv_path):
        with open(os.path.join(os.path.dirname(args.csv_path), "dnsmos_describe.out"), "w") as wf:
            print(pd.read_csv(args.csv_path).describe(), file=wf)
        exit()
    models = glob.glob(os.path.join(args.testset_dir, "*"))
    audio_clips_list = []
    p808_model_path = os.path.join(args.dnsmos_onnx_home, 'model_v8.onnx')
    primary_model_path = os.path.join(args.dnsmos_onnx_home, 'sig_bak_ovr.onnx')
    ...
    print(f"audio num: {len(clips)}")
    desired_fs = SAMPLING_RATE
    assert len(clips) == 1200
...
if __name__=="__main__":
    ...
    parser.add_argument('-d', "--dnsmos_onnx_home", default=None, help='Dir to the DNS models')
