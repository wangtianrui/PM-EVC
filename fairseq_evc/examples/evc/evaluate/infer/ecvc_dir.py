...
if __name__ == "__main__":
    ...
    for idx, line in enumerate(tqdm(lines)):
        ...
        with torch.no_grad():
            ...
            if args.task == "vc":
                result = model.decode([src_embs[0], tgt_embs[1], src_embs[2], src_embs[3]], src_len)
                save_path = os.path.join(args.out_home, "npy", "vc", save_name)
            if args.task == "ec":
                result = model.decode([src_embs[0], src_embs[1], tgt_embs[2], src_embs[3]], src_len)
                save_path = os.path.join(args.out_home, "npy", "ec", save_name)
            if args.task == "ecvc":
                result = model.decode([src_embs[0], tgt_embs[1], tgt_embs[2], src_embs[3]], src_len)
                save_path = os.path.join(args.out_home, "npy", "ecvc", save_name)
            if args.task == "rec":
