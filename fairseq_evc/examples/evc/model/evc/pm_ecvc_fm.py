...
from .flow_matching import FlowMatchingModule
...
class PMECVCConfig(FairseqDataclass):
    ...
    decoder_fm_attention_head_dim: int = field(default=64)
    decoder_fm_n_blocks: int = field(default=4)
    decoder_fm_num_mid_blocks: int = field(default=12)
    decoder_fm_num_heads: int = field(default=8)
    decoder_fm_act_fn: str = field(default="gelu")
    decoder_fm_channels: str = field(default="256;256")
...
@register_model("pm_ecvc_fm", dataclass=PMECVCConfig)
class PMECVC(BaseFairseqModel):
    def __init__(self, cfg: PMECVCConfig, asr_dict):
        ...
        self.fm = FlowMatchingModule(
            spk_emb_dim=cfg.spk_hidden_dim, emo_emb_dim=cfg.emo_hidden_dim, 
            n_feats=cfg.n_mel, decoder_config={
                'channels': map(int, cfg.decoder_fm_channels.split(";")), 'dropout': 0.05, 
                'attention_head_dim': cfg.decoder_fm_attention_head_dim, 
                'n_blocks': cfg.decoder_fm_n_blocks, 
                'num_mid_blocks': cfg.decoder_fm_num_mid_blocks, 
                'num_heads': cfg.decoder_fm_num_heads, 
                'act_fn': cfg.decoder_fm_act_fn
            }
        )
    ...
    def forward(
        ...
        *args, **kwargs
        ...
        if only_downstream:
            ...
            diff_loss = self.fm(
                coarse_est=decoder_out[:, :max_len].detach(),
                spk=spk_pooled[:, :max_len].detach(), 
                emo=emo_pooled[:, :max_len].detach(), 
                tgt_fbank=tgt_fbank, 
                x_mask=(1-padding_mask.to(content_feature.dtype))[:, :max_len].unsqueeze(1),
                feat_len=torch.clamp(feature_len, max=max_len),
                conv_out=None
            )
    ...
    def encode(self, source, pitch, padding_mask=None, feature_len=None):
        ...
        content_feature, spk_pooled, emo_pooled, con_4_ctc, spk_4_ce, emo_4_ce, spk_pooled_utt, emo_pooled_utt = \
                self.forward_downstream(stacked_feature, feature_len, padding_mask)
        feature = [content_feature, spk_pooled_utt, emo_pooled_utt, conv_out, padding_mask]
        return feature
    
    def decode(self, stacked_feature, n_timesteps, temperature, tgt_conv=None, feature_len=None, only2stage=False):
        content_feature, spk_pooled_utt, emo_pooled_utt, conv_out, padding_mask = stacked_feature
        if tgt_conv is None:
            tgt_conv = conv_out[:, :content_feature.size(1)]
        ...
        decoder_out = decoder_out_first + compensate
        # print(decoder_out)
        decoder_out = decoder_out.permute(0, 2, 1)
        max_len = decoder_out.size(1)
        if max_len % 2 != 0:
            max_len = max_len - 1
        decoder_out = self.fm.synthesise(
            decoder_out[:, :max_len], spk_pooled[:, :max_len], emo_pooled[:, :max_len], 
            x_mask=(1-padding_mask.to(content_feature.dtype))[:, :max_len].unsqueeze(1),
            n_timesteps=n_timesteps, temperature=temperature, 
            conv_f=None,
            feat_len=feature_len
        )
        return decoder_out