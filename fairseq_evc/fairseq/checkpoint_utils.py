    ...
    for filename in filenames:
        ...
        for shard_idx in range(num_shards):
            ...
            if "fsdp_metadata" in state and num_shards > 1:
                ...
                if "_name" not in cfg.model:
                    OmegaConf.set_struct(cfg.model, False)
                    cfg.model._name = cfg.model["arch"]
                print(cfg.model)
                model.load_state_dict(
...
def load_pretrain_checkpoint(model, checkpoint_path):
    ...
    for k, v in model_state_dict.items():
        if k.find("adapter_") != -1 and k.find("ln") != -1 and k not in pretrained_dict.keys():
            pretrained_dict[k] = state["model"][k.replace("adapter_", "").replace(".ln", "")]

    model_state_dict.update(pretrained_dict)
    model.load_state_dict(model_state_dict)
    logging.info("loaded pretrained checkpoints: %s" % str(pretrained_dict.keys()))
    return model
